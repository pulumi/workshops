...
session times, or speakers assigned as of this build). Both the demo code and
the slide deck are built on this branch. See `README.md` for the demo layout
and how to run it, and `slides/AGENTS.md` for the deck's own brief.
...
Built in `slides/` on this same branch (Slidev with `@pulumi/slidev-theme`),
matching the demo flow above step for step. See `slides/AGENTS.md` for the
deck's own brief, sources, and deviations.
