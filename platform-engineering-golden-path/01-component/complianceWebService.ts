...
export class ComplianceWebService extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: ComplianceWebServiceArgs, opts?: pulumi.ComponentResourceOptions) {
        super("platform-golden-path:index:ComplianceWebService", name, args, opts);

        const mandatoryTags = {
            "platform:managed-by": "compliance-web-service-component",
            "platform:service": args.serviceName,
        };
        ...
        new aws.iam.RolePolicy(`${name}-task-role-logs`, {
            role: taskRole.id,
