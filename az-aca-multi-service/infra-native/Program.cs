...
return await Pulumi.Deployment.RunAsync(() =>
{
            ...
            Containers = new List<Pulumi.AzureNative.App.Inputs.ContainerArgs>
            {
                new Pulumi.AzureNative.App.Inputs.ContainerArgs
                {
                    ...
                    Env = new List<Pulumi.AzureNative.App.Inputs.EnvironmentVarArgs>
                    {
                        ...
                        new Pulumi.AzureNative.App.Inputs.EnvironmentVarArgs { Name = "FailureRate", Value = "0.3" }  // 30% failure rate to demonstrate retries
                    },
    ...
    // Get backend internal FQDN (for frontend to call backend)
    var backendUrl = backendApp.Configuration.Apply(c =>
        c != null && c.Ingress != null && c.Ingress.Fqdn != null
            ? $"https://{c.Ingress.Fqdn}"
            : "");

    // Add resiliency policy to backend
    var backendResiliency = new AppResiliency("backendResiliency", new AppResiliencyArgs
    {
        Name = "backend-resiliency-policy",
        AppName = backendApp.Name,
        ResourceGroupName = resourceGroup.Name,
        HttpRetryPolicy = new Pulumi.AzureNative.App.Inputs.HttpRetryPolicyArgs
        {
            MaxRetries = 3,
            // Backoff properties are directly on HttpRetryPolicyArgs
            InitialDelayInMilliseconds = 500,
            MaxIntervalInMilliseconds = 5000,
            // Matching criteria are directly on HttpRetryPolicyArgs
            // Must include 'retriable-status-codes' when using HttpStatusCodes
            Errors = new InputList<string> { "5xx", "reset", "connect-failure", "retriable-status-codes" },
            HttpStatusCodes = new InputList<int> { 502, 503, 504 }
        },
        TimeoutPolicy = new Pulumi.AzureNative.App.Inputs.TimeoutPolicyArgs
        {
            ResponseTimeoutInSeconds = 30,
            ConnectionTimeoutInSeconds = 5
        },
        CircuitBreakerPolicy = new Pulumi.AzureNative.App.Inputs.CircuitBreakerPolicyArgs
        {
            ConsecutiveErrors = 5,
    ...
    // Build frontend image (BFF will proxy to backend at runtime)
        ...
        Template = new Pulumi.AzureNative.App.Inputs.TemplateArgs
        {
            Containers = new List<Pulumi.AzureNative.App.Inputs.ContainerArgs>
            {
                new Pulumi.AzureNative.App.Inputs.ContainerArgs
                {
                    ...
                    Env = new List<Pulumi.AzureNative.App.Inputs.EnvironmentVarArgs>
                    {
                        new Pulumi.AzureNative.App.Inputs.EnvironmentVarArgs { Name = "BACKEND_URL", Value = backendUrl }
                    },
                    Probes = new List<Pulumi.AzureNative.App.Inputs.ContainerAppProbeArgs>
                    {
                        new Pulumi.AzureNative.App.Inputs.ContainerAppProbeArgs
